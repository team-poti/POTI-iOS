...
enum ParticipantStatus: String {
    ...
    func statusText(role: UserRole) -> String {
        switch self {
        case .recruiting:
            return role == .host ? "참여자들을 기다리고 있어요" : "다른 참여자들을 기다리고 있어요"
            
        case .waitPay:
            return role == .host ? "입금을 기다리는 중이에요" : "지금 입금해주세요!"
            
        case .waitPayCheck:
            return role == .host ? "입금 확인을 기다리는 참여자가 있어요" : "모집자가 입금 내역을 확인하고 있어요"
            
        case .paid:
            return role == .host ? "배송을 기다리는 참여자가 있어요" : "모집자가 배송을 준비 중이에요"
            
        case .shipped:
            return role == .host ? "배송을 시작했어요" : "모집자가 배송을 시작했어요"
